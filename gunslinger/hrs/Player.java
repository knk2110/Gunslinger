...
public class Player extends gunslinger.sim.Player
{
    ...
    public void init(int nplayers, int[] friends, int enemies[])
    {
		...
		if(createLog) try {
			FileWriter fstream = new FileWriter("gunslinger/hrs/log.txt");
	...
	private final double OPPONENT_FACTOR     = +0.0;
	private final double RETALIATION_FACTOR  = +1.0;
	private final double INERTIA_FACTOR      = -0.3;
	private final double SUPPORT_FACTOR      = +0.0;
	private final double DISPERSION_FACTOR   = -0.0;
	private final double HISTORIC_FACTOR     = +0.0;
	...
	private double[] getExpectedShots(boolean[] alive)
	{
		...
		for(int r = 0; r < oldRounds; r++)
			for(int i = 0; i < nplayers; i++)
					// If A shot B in round (roundNum -r -1) and B is alive, shooter of B in that round is A :)
		...
		for(int i = 0; i < nplayers; i++)
			if(alive[i])
			{
				if(shooter[0][i] != -1 && shooter[0][i] != id) // If the player was shot last round
				{
					expectedShots[i] = expectedShots[i] + REPEAT_FACTOR;
					if(shooter[0][shooter[0][i]] != -1) // If the player's shooter was shot by someone else
						expectedShots[i] = expectedShots[i] + DEFENSE_FACTOR;
					
					// If the player's opponents leverage the opportunity
					// expectedShots[i] = expectedShots[i] + f(OPPONENT_FACTOR); 
				}
				if(history[roundNum -1][i] != -1 && history[roundNum -1][i] != id && alive[history[roundNum -1][i]])
				{
					expectedShots[i] = expectedShots[i] + RETALIATION_FACTOR; // If the player shot someone
					int target = history[roundNum -1][i];
					if(history[roundNum -1][target] != -1 && alive[history[roundNum -1][target]])
						expectedShots[i] = expectedShots[i] + INERTIA_FACTOR; // If the player's target was shooting someone else
					
					// If the target's friends retaliate against the player
					// expectedShots[i] = expectedShots[i] + f(SUPPORT_FACTOR); 
				}
				
				//If too many players have fired shots and the targets are still alive
				// expectedShots[i] = expectedShots[i] + f(DISPERSION_FACTOR);
				
				//If the player was shot, or the player had shot someone in previous rounds
				// expectedShots[i] = expectedShots[i] + f(HISTORIC_FACTOR);
			}
			
		// ###Important -> Should the sum of expected shots be <= playersAlive ?
